...
def what_if(session_id: str = "default", description: str = "") -> str:
    ...
    return (
        ...
        "   - `forcing_set_forcing` for rainfall or inflow changes.\n"
        "   - `editing_set_node_properties`, "
        "`editing_set_link_properties`, or "
        "`editing_set_subcatchment_properties` for parameter changes "
        "on existing elements.\n"
        "   - `spatial_add_lid` for green-infrastructure additions.\n"
        "   - `spatial_set_treatment` for water-quality modifications.\n"
...
def build_simple_model(description: str) -> str:
    ...
    return (
        ...
        "8. Save the model with `building_write_model`.\n"
